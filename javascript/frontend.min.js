"use strict";class Node{constructor(array){this.node=this.createSingleNode(array);}
createSingleNode(array){let node=document.createElement(array[0]);for(let i=1,l=array.length;i<l;i++){if(array[i]instanceof Array){node.appendChild(this.createSingleNode(array[i]));}
if(typeof array[i]==="object"){let keys=Object.keys(array[i]);for(let i=0,l=keys.length;i<l;i++)node[keys[i]]=array[i][keys[i]];continue;}
